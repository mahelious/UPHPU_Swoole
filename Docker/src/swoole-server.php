...
// https://github.com/TechEmpower/FrameworkBenchmarks/blob/master/frameworks/PHP/swoole/swoole.dockerfile

use Swoole\Http\Request;
use Swoole\Http\Response;
$server = new swoole_http_server('0.0.0.0', 9502, SWOOLE_BASE);
$server->set([
    'worker_num' => NUMCORES
]);
...
$db = function (string $database_type, int $queries = 0) use ($pool): string {
    ...
    while (0 < $query_count--) {
        ...
        $ret = $db->query("SELECT randomNumber FROM world WHERE id = $id");
...
$updates = function (string $database_type, int $queries = 0) use ($pool): string {
    ...
    while (0 < $query_count--) {
        ...
        $ret = $db->query("UPDATE World SET randomNumber = $randomNumber WHERE id = $id");
        $arr[] = $world;
    }
    $pool->put($db);
    return json_encode($arr);
